from conversor import getHuaweiCloudData
import math

initialStr = """function getULs(){
...
def jsCodeGenerator(buyList, region):
    res = initialStr

    res += "(async ()=>{"
    for option in buyList:
        if option["kind"] == "ecs":
            flavorInfo = getHuaweiCloudData(option["type"], region)
            if flavorInfo["family"] == "flexus":
                res += f"await addFlexus('{region}', {flavorInfo['vCPUs']}, {flavorInfo['memory']}, {option['usage']})\n"
            else:
                res += f"await addECS('{region}', 'ri', 'x86', '{flavorInfo['family']}', '{option['type'].split('.')[0]}', {flavorInfo['vCPUs']}, {flavorInfo['memory']}, {option['usage']})\n"
        elif option["kind"] == "evs":
            res += f"await addEVS('{region}', '{'High I/O' if option['type'] == 'HighIO' else 'General Purpose SSD'}', {math.ceil(option['usage'])}, 1)\n"
